...
import type { LoginFormStoreValues, LoginFormStoreProps } from "../../types/auth-types";

const LoginFormStore: React.FC<LoginFormStoreProps> = ({ onSubmit }) => {
  const [showPassword, setShowPassword] = useState(false);
  const [serverError, setServerError] = useState("");

  const initialValues: LoginFormStoreValues = {
    email: "",
    password: "",
  };

  const handleSubmit = async (
    values: LoginFormStoreValues,
    { setSubmitting }: FormikHelpers<LoginFormStoreValues>
  ) => {
    try {
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-lg shadow-xl">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-3xl font-bold text-gray-900">
            Store Admin Admin Portal
          </h2>
          <p className="mt-2 text-sm text-gray-600">
            Secure login portal for store admin control and management
          </p>
        </div>
