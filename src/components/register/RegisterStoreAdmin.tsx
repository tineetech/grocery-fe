...
import type { RegisterFormStoreAdminProps, RegisterFormStoreAdminValues } from "../../types/auth-types";
import { registerSchema } from "@/helper/validation-schema-register";

const RegisterStoreAdmin: React.FC<RegisterFormStoreAdminProps> = ({ onSubmit }) => {
  const [serverError, setServerError] = useState("");

  const initialValues: RegisterFormStoreAdminValues = {
    email: "",
  };

  const handleSubmit = async (
    values: RegisterFormStoreAdminValues,
    { setSubmitting }: FormikHelpers<RegisterFormStoreAdminValues>
  ) => {
    try {
...
export default RegisterStoreAdmin;
